...
zooid doesn't run its own media server. Audio and video calls are handled by [LiveKit](https://livekit.io/), a separate open-source WebRTC toolkit, and zooid just wires your relay into it — creating rooms, tracking who's in a call, and publishing presence so Flotilla knows a call is live. That means before calls work at all, you need a LiveKit server: either a self-hosted instance or a LiveKit Cloud project. From there you'll need three values to hand to zooid: the server's `server_url`, an `api_key`, and an `api_secret`.
...
Watch out for one silent failure mode: if `api_key` is left blank, zooid doesn't refuse to start or log a warning — the call-related endpoints simply return 404 to every request. If members report that voice rooms exist but calls never connect, check this block first.

## Point a webhook back at your relay

LiveKit needs to tell your relay when someone joins or leaves a call, so it can publish presence and let Flotilla reflect who's actually in a room. On the LiveKit side, configure a webhook pointing at:
...
(If you're running several relays that all share one LiveKit project, LiveKit only supports a single webhook URL, so point it at the management API's shared webhook endpoint instead of any one relay's own URL — but that requires `API_HOST` to be configured. For a single relay, the per-relay URL above is simpler and is what most self-hosters should use.)

## Opt a room in

Configuring `[livekit]` makes calling available to the relay, but it doesn't turn every room into a call. Each room that should support calls needs a `livekit` tag in its NIP-29 group metadata — in practice, this happens automatically when an admin creates or edits a room in Flotilla and chooses the "Voice" room type. That option only appears in the room-creation form once the relay advertises LiveKit support, so getting the config right on the relay side is the prerequisite for the option showing up at all. One LiveKit room maps to exactly one room in your space — calls don't span rooms or spaces.

## If your space is hosted, not self-hosted

If your space runs on [Coracle Hosting](/articles/coracle-hosting) rather than a relay you manage yourself, you don't touch any of the above directly. Video calling is one of the relay feature toggles on your relay's page in the hosting panel (see [Managing Your Hosted Relay](/articles/managing-your-hosted-relay)), and it's only available on paid plans — the Basic and Growth plans include it, while the Free plan does not. Coracle runs and configures the LiveKit server for you; flipping the toggle is the entire setup.

## Verifying it works

Once `[livekit]` is configured and the webhook is pointed at your relay, create or edit a room in Flotilla and confirm "Voice" appears as a room type. If it doesn't show up, double-check that `api_key` isn't empty and that the config file reloaded cleanly — zooid hot-reloads config changes, but a typo in the file will leave that relay unreachable until it's fixed. If the room type appears but calls fail to connect once you join, that usually points to a webhook signature mismatch rather than the `[livekit]` block itself.
