...
Money enters the picture in just one place: when *you* want to run your own space rather than join someone else's. That's separate from using Flotilla, because Flotilla doesn't host anything — creating a space in the app sends you to a hosting choice, not to a relay Flotilla spins up for you. See [Spaces and Rooms Explained](/articles/spaces-and-rooms-explained) for that mental model.
...
The one place you might choose to spend money inside Flotilla is [zaps](/articles/zaps-and-wallet) — small Lightning tips you can send on a message, a thread post, or a community fundraising goal. Zaps are entirely opt-in: nothing in Flotilla requires you to send or receive one, and plenty of people use the app for years without ever connecting a wallet. On iOS, zaps and the wallet are turned off entirely due to Apple's App Store policy, so the option isn't even present on that platform.
