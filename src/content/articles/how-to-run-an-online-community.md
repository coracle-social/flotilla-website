...
Getting started takes just a few minutes. Head to [app.flotilla.social](https://app.flotilla.social) and click "Create an Account." Flotilla allows you to sign up using email and password, or by generating a "key". Unlike a normal login, this identity really belongs to you, not to any platform, so no one else's decision can lock you out. (It runs on an open technology called Nostr, but you never have to think about that to use Flotilla — if you're curious how it works under the hood, see [What is Nostr?](/articles/what-is-nostr).) If you decide to use this method, keep your backup somewhere safe, like a password manager. If you lose it, you lose your account.
...
By default, a new space is locked down. Until you say otherwise, only people you specifically invite or add can read or post — exactly what you want while you're getting your first members in. The simplest way to bring someone in is by using an invite link, created from the space's main menu.
...
Either way, you can give your community its own web address. A managed space comes with a default address out of the box (something like `myspace.spaces.coracle.social`), and you can swap in your own domain — `chat.mycommunity.com`, say — by adding one DNS record with whoever manages your domain.
...
Moderation gets easier when you are not doing it alone, and Flotilla gives you a few tools to share the load. Open your space's member list from the main menu — you can add people from there as well. You can also remove someone from the space, or ban them, preventing them from re-joining. Anyone you ban can be restored later from the same screen, so a ban isn't permanent unless you want it to be.
...
The first extra is media storage: where the images and files your members share actually get kept. If you run your own server you can store them on the same machine your community lives on, or hand them off to a cloud storage service instead. One thing worth knowing: by default, uploaded files can be opened by anyone who has the link, even people outside your community. If your space is meant to be private, be sure to avoid sharing media links outside the community.

The second is voice and video calls. Calls with screen sharing are supported, but they rely on a separate piece of software that handles the live audio and video. On your own server you set that up yourself; on managed hosting it's one toggle and the rest is done for you.
